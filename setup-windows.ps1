...
function Fetch-Repo($Owner, $Name, $Commit, $Sha256, $Marker, [switch]$Required) {
    if (Test-Path $Marker) {
...
if (-not (Test-Path $uv)) {
    Write-Host "[1/8] Descargando uv..." -ForegroundColor Yellow
    ...
    Invoke-WebRequest $uvAsset.browser_download_url -UseBasicParsing -OutFile $uvZip
...
Write-Host "[3/8] Construyendo la release instalada..." -ForegroundColor Yellow
...
Write-Host "[5/8] Preparando modelos auxiliares..." -ForegroundColor Yellow
...
Fetch-Repo "omine-me" "LaughterSegmentation" "a525292d26f744e14624e3a2f1fb5e3c7858d7b3" `
    ...
    (Join-Path $components "LaughterSegmentation\train\model.py") -Required
...
Write-Host "[6/8] Preparando ffmpeg..." -ForegroundColor Yellow
...
if (-not (Test-Path (Join-Path $ffdir "ffmpeg.exe"))) {
    ...
    Invoke-WebRequest $ffAsset.browser_download_url -UseBasicParsing -OutFile $ffz
...
Write-Host "[7/8] Preparando llama.cpp Vulkan..." -ForegroundColor Yellow
...
if (-not (Test-Path (Join-Path $llamaDir "llama-server.exe"))) {
    try {
        ...
        if ($asset) {
            ...
            Invoke-WebRequest $asset.browser_download_url -UseBasicParsing -OutFile $llamaZip
...
# 8. Verificación integral antes de declarar la instalación utilizable.
Write-Host "[8/8] Verificando runtime, dependencias y herramientas..." -ForegroundColor Yellow
& $bootstrapPython (Join-Path $PSScriptRoot "tools\verify_windows_install.py") --root $PSScriptRoot
Assert-Ok "La verificacion final de la instalacion fallo"
